...
function walk(dir, fileList = []) {
  ...
  for (const it of items) {
    ...
    if (it.isDirectory()) {
      if (['node_modules', '.next', '.git', 'dist', 'build', '.parcel-cache', '.turbo'].includes(it.name)) continue;
      walk(full, fileList);
    } else {
      fileList.push(full);
    }
...
function isSource(file) {
  return exts.includes(path.extname(file)) && file.includes(path.sep + 'pup-focus' + path.sep);
}
...
function resolveImport(fromFile, imp) {
  if (imp.startsWith('.') || imp.startsWith('/')) {
    ...
    const tries = [candidate].concat(exts.map(e => candidate + e), exts.map(e => path.join(candidate, 'index' + e)));
...
function main() {
  ...
  const entryPoints = allFiles.filter(f => {
    ...
    return rel.startsWith('pup-focus/app/') || rel.startsWith('pup-focus/pages/') || rel.includes('/components/') || rel.includes('/layout') || rel.endsWith('page.tsx') || rel.endsWith('layout.tsx');
  ...
  const filtered = candidates.filter(c => !c.includes(path.sep + 'scripts' + path.sep) && !c.endsWith('test.ts') && !c.endsWith('.d.ts'));
