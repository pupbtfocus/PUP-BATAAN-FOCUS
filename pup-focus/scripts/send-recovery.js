...
async function main() {
  ...
  const to = args[0];
  if (!to) {
    console.error("Usage: node scripts/send-recovery.js <recipient-email>");
    process.exit(1);
  }
  ...
  const subject = "PUP FOCUS — Account recovery";
  const text = `Hello,\n\nA recovery link has been requested for your account. Click the link to set a password and sign in:\n\n${actionLink}\n\nOTP: ${otp || "(none)"}\n`;
  const html = `<p>Hello,</p><p>A recovery link has been requested for your account.</p><p><a href="${actionLink}">Reset / set your password</a></p><p>OTP: ${otp || "(none)"}</p>`;
